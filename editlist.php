...
<form class="add_form" method="post" action="editlist.php">
    ...
    <div class="form-group">
        <label for="sights">What do you want to see/do there?</label>
            <input type="text" class="form-control" name="sights" value="
            <?php 
            if (!empty($id)){
                echo $sights;
            } else {
...
<div class="container read_list"> 
    ...
    foreach(get_full_list() as $item){
        echo '<div class="row">' 
        . "<li class='col-xs-10 col-md-10'> <a href='single.php?id=" . $item["key"] . "'>" 
        . '<img class="list" src="img/' . $item['image'] . '">' 
        . $item['city'] . ", " 
        . $item['country'] . '</li>';
            
 // Load ID'ed item into edit fields
        echo '<div class="col-xs-2 col-md-2">' 
        . '<a class="edit" href="editlist.php?id=' 
        . $item["key"] 
        . '"> Edit  </a>' . "/";
            
//redirect to Are you sure? Delete page for specific item
        echo '<a class="delete" href="delete.php?id=' 
        . $item["key"] 
        . '"> Delete</a></div></div>';   
    } ?>
</div>
